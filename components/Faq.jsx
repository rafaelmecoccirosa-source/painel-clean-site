...
function Faq() {
  ...
  return (
    <section id="faq" style={{ background: "#F6F1DE", padding: "120px 0" }}>
      <div className="container">
        <div className="faq-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0,0.8fr) minmax(0,1.2fr)", gap: 80, alignItems: "flex-start" }}>
          <div className="faq-side" style={{ position: "sticky", top: 100 }}>
            <div className="eyebrow" style={{ color: "var(--pc-green-2)", marginBottom: 18 }}>Perguntas frequentes</div>
            <h2 className="display" style={{ fontSize: "clamp(36px, 4.4vw, 56px)", lineHeight: 1.05, color: "var(--fg-on-cream-1)", margin: "0 0 24px" }}>O essencial,<br /><span style={{ color: "var(--fg-on-cream-3)" }}>antes de comprar.</span></h2>
          ...
          <div>
            {faqs.map((f, i) => (
              <div key={i} style={{ borderBottom: "1px solid var(--border-on-cream)" }}>
                <button onClick={() => setOpen(open === i ? -1 : i)} style={{ width: "100%", padding: "26px 0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, textAlign: "left", color: "var(--fg-on-cream-1)" }}>
                  <span className="display" style={{ fontSize: "clamp(18px, 2vw, 22px)", fontWeight: 500, letterSpacing: "-0.01em" }}>{f.q}</span>
                  <span style={{ flexShrink: 0, width: 36, height: 36, borderRadius: "50%", background: open === i ? "var(--pc-green)" : "transparent", border: open === i ? "1px solid var(--pc-green)" : "1px solid var(--border-on-cream)", color: "var(--fg-on-cream-1)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 240ms", transform: open === i ? "rotate(45deg)" : "rotate(0)" }}>
                    <Icon name="plus" size={16} />
                  </span>
                </button>
                <div style={{ maxHeight: open === i ? 240 : 0, overflow: "hidden", transition: "max-height 360ms ease, opacity 240ms ease", opacity: open === i ? 1 : 0, paddingBottom: open === i ? 28 : 0 }}>
                  <p style={{ margin: 0, paddingRight: 60, font: "var(--body)", color: "var(--fg-on-cream-2)", maxWidth: 720 }}>{f.a}</p>
                </div>
              </div>
            ))}
          </div>

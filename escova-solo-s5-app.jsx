...
function StickyBuyBar({ p }) {
  ...
  return (
    <div style={{ position: "fixed", top: 76, left: 0, right: 0, zIndex: 50, background: "rgba(12,40,31,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid var(--border-on-dark)", transform: visible ? "translateY(0)" : "translateY(-100%)", transition: "transform 300ms var(--ease-out)", padding: "12px 0" }}>
    ...
    <section style={{ background: "var(--pc-cream)", padding: "80px 0" }}>
      <div className="container">
        ...
        <div className="specs-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {p.specs.map((s, i) => (
            <div key={i} style={{ padding: "24px", background: "white", border: "1px solid var(--border-on-cream)", borderRadius: "var(--radius-xl)" }}>
      ...
      <div className="container">
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 500 }}>
            <thead>
              <tr>
                <th style={{ padding: "14px 20px", textAlign: "left", fontSize: 12, fontWeight: 600, color: "var(--fg-4)", textTransform: "uppercase", letterSpacing: "0.1em" }}></th>
                {allProducts.map((pr, i) => (
                  <th key={i} style={{ padding: "14px 20px", textAlign: "center", background: pr.current ? "var(--accent-fill)" : "rgba(255,255,255,0.03)", border: "1px solid " + (pr.current ? "var(--border-accent)" : "var(--border-on-dark)"), borderBottom: "none", color: pr.current ? "var(--pc-green)" : "white", fontSize: 15, fontWeight: 600, borderRadius: "var(--radius-md) var(--radius-md) 0 0" }}>
                    <a href={pr.href} style={{ color: "inherit" }}>{pr.name}</a>
              ...
              {compareRows.map((row, ri) => (
                <tr key={ri}>
                  ...
                  {allProducts.map((pr, ci) => (
                    <td key={ci} style={{ padding: "14px 20px", textAlign: "center", fontSize: 14, fontWeight: 600, color: pr.current ? "var(--pc-green)" : "var(--fg-2)", background: pr.current ? "var(--accent-fill-soft)" : "transparent", border: "1px solid " + (pr.current ? "var(--border-accent)" : "var(--border-on-dark)") }}>
                      {row.vals[ci]}
                    </td>
                  ))}
                </tr>
                ...
                {allProducts.map((pr, i) => (
                  <td key={i} style={{ padding: "14px 20px", textAlign: "center", background: pr.current ? "var(--accent-fill-soft)" : "transparent", border: "1px solid " + (pr.current ? "var(--border-accent)" : "var(--border-on-dark)"), borderTop: "none", borderRadius: "0 0 var(--radius-md) var(--radius-md)" }}>
                    <a href={wa(`Olá! Tenho interesse no modelo ${pr.name} (${pr.sku}).`)} target="_blank" rel="noopener noreferrer"
                      style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, padding: "9px 14px", borderRadius: "var(--radius-md)", background: pr.current ? "var(--pc-green)" : "rgba(255,255,255,0.06)", color: pr.current ? "var(--pc-dark)" : "var(--fg-2)" }}>
    ...
    <div style={{ background: "var(--pc-deepest)", borderTop: "1px solid var(--border-on-dark-soft)", padding: "24px 0" }}>
      <div className="container" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <a href="/produtos" style={{ fontSize: 13, color: "var(--fg-4)", transition: "color 200ms" }}
          ...
          onMouseLeave={e => e.currentTarget.style.color = "var(--fg-4)"}>Ver todos os produtos</a>
        {p.next ? (
